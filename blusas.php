...
$nombre_archivo_actual = basename($_SERVER['PHP_SELF']); // blusas.php
$id_categoria_actual_page = 1; // ID de la categoría "Blusas"
$nombre_pagina_actual = "Blusas";
...
// Para Blusas, no hay filtro de sesión por defecto.
...
if (isset($_GET['accion']) && $_GET['accion'] == 'obtener_detalles_producto' && isset($_GET['id_producto'])) {
    ...
    $respuesta = ['error' => 'No se pudieron obtener los detalles de la blusa.'];
    if ($id_producto_solicitado > 0 && isset($conn)) {
        try {
            ...
            } else { $respuesta = ['error' => 'Blusa no encontrada o no pertenece a esta categoría.']; }
...
if (isset($conn)) {
    ...
    $stmt_col_ui->bind_param("i", $id_categoria_actual_page);
    ...
    $stmt_rango_p->bind_param("i", $id_categoria_actual_page);
    ...
    // Para blusas, no se añade filtro de sesión (id_sesion_filtro es null)
    if ($precio_seleccionado_filtro !== null) {
