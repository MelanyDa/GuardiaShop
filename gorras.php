...
$nombre_archivo_actual = basename($_SERVER['PHP_SELF']); // gorras.php
$id_categoria_actual_page = 3; // ID de la categoría "Gorras"
$nombre_pagina_actual = "Gorras";
...
// Para Gorras, no hay filtro de sesión por defecto.
$id_sesion_filtro = null; 
// Si en el futuro las gorras tuvieran filtro de sesión, esta línea lo leería:
...
if (isset($_GET['accion']) && $_GET['accion'] == 'obtener_detalles_producto' && isset($_GET['id_producto'])) {
    ...
    $respuesta = ['error' => 'No se pudieron obtener los detalles de la gorra.'];
    if ($id_producto_solicitado > 0 && isset($conn)) {
        try {
            ...
            } else { $respuesta = ['error' => 'Gorra no encontrada o no pertenece a esta categoría.']; }
...
if (isset($conn)) {
    $sql_colores_ui = "SELECT DISTINCT cp.id_color, cp.nombre, cp.codigo_hexadecimal FROM color_productos cp JOIN detalles_productos dp ON cp.id_color = dp.id_color JOIN productos p ON dp.id_producto = p.id_producto WHERE p.id_categoria = ? ORDER BY cp.nombre ASC";
    $stmt_col_ui = $conn->prepare($sql_colores_ui);
    $stmt_col_ui->bind_param("i", $id_categoria_actual_page); // Solo filtra por categoría
    ...
    $sql_rango_p = "SELECT MIN(dp.precio_producto) as min_p, MAX(dp.precio_producto) as max_p FROM detalles_productos dp JOIN productos p ON dp.id_producto = p.id_producto WHERE p.id_categoria = ?";
    $stmt_rango_p = $conn->prepare($sql_rango_p);
    $stmt_rango_p->bind_param("i", $id_categoria_actual_page); // Solo filtra por categoría
