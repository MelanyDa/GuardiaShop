...
$nombre_archivo_actual = basename($_SERVER['PHP_SELF']);
$id_categoria_actual_page = null; // Para shop.php, no hay categoría específica por defecto
$nombre_pagina_actual = "Productos";

// --- Leer parámetros GET para los filtros (igual que antes) ---
...
if (isset($_GET['accion']) && $_GET['accion'] == 'obtener_detalles_producto' && isset($_GET['id_producto'])) {
    ...
    $respuesta = ['error' => 'No se pudieron obtener los detalles del producto.'];
    if ($id_producto_solicitado > 0 && isset($conn)) {
        try {
            $sql_base_modal = "SELECT p.id_producto, p.nombre, p.descripcion FROM productos p WHERE p.id_producto = ?";
            // Para shop.php, no filtramos por categoría en el modal por defecto, ya que puede ser cualquier producto
            $stmt_base = $conn->prepare($sql_base_modal);
            $stmt_base->bind_param("i", $id_producto_solicitado);
            ...
            } else { $respuesta = ['error' => 'Producto no encontrado.']; } // Mensaje genérico para shop.php
...
if (isset($conn)) {
    // Para shop.php, los colores y rango de precios son globales o según filtro de sesión si aplica
    ...
    // En shop.php, $id_categoria_actual_page es null, así que no se añade filtro de categoría aquí.
    ...
    // Para shop.php, $id_categoria_actual_page es null, no se añade filtro de categoría a menos que venga de un enlace específico (no implementado aquí)
...
<head>
    <meta charset="UTF-8">
    <meta name="description" content="Tienda GuardiaShop - <?php echo htmlspecialchars($nombre_pagina_actual); ?>">
...
<body>
    ...
    <main class="main-content">
        ...
        <h1 class="page-title"><?php echo strtoupper(htmlspecialchars($nombre_pagina_actual)); ?></h1>
    ...
    <div class="filtro-ropa-opciones">
        ...
        <a href="gorras.php" class="filtro-ropa-btn <?php if ($nombre_archivo_actual == 'gorras.php') echo 'active'; ?>">Gorras</a>
                ...
                <?php // Para shop.php, el filtro de género es opcional y general
                // Para camisetas.php y short.php, se mostrará siempre.
                // Para blusas.php y gorras.php, no se mostrará (a menos que cambies esta lógica).
                if ($nombre_archivo_actual == 'shop.php' || $nombre_archivo_actual == 'short.php' || $nombre_archivo_actual == 'camisetas.php'): 
                    $current_sesion_filter_ui = $id_sesion_filtro; // Usar la variable ya leída del GET
                ?>
    ...
    <div class="price-filter" style="display: flex; align-items: center; gap: 12px;">
        ...
        ?>
        <a href="<?php echo $urlTodosPrecios; ?>" class="filter-options-btn <?php if($precio_seleccionado_filtro === null) echo 'active-filter';?>" style="margin-right: 10px;">Ver Todos</a>
        <input type="range" name="precio_sel" min="<?php echo $precio_min_db_slider; ?>" max="<?php echo $precio_max_db_slider; ?>" 
...
<details id="detailsFiltroColor">
    ...
    <div class="filter-color-options color-picks" style="display: flex; align-items: center; gap: 12px;">
        ...
        ?>
        <a href="<?php echo $urlTodosColores; ?>" class="filter-options-btn <?php if ($id_color_filtro === null) echo 'active-filter'; ?>" style="margin-right: 10px;">Todos</a>
        <?php foreach ($colores_para_filtro_ui as $color_f): ?>
            <?php
